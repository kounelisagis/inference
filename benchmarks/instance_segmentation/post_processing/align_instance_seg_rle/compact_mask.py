...
def rle_encode(mask_2d: torch.Tensor) -> npt.NDArray[np.int32]:
    ...
    Examples:
        ```pycon
        >>> import numpy as np
        >>> from supervision.detection.compact_mask import rle_encode
        >>> mask = np.array([[False, True, True], [True, False, False]])
        >>> rle_encode(mask).tolist()
        [1, 2, 1, 1, 1]
...
def _rle_decode(
    ...
    Args:
        rle: int32 array of run lengths as produced by :func:`rle_encode`.
    ...
    def repack(self) -> CompactMask:
        ...
        for mask_idx in range(num_masks):
            ...
            if not rows_any.any():
                ...
                new_rles.append(rle_encode(np.zeros((1, 1), dtype=bool)))
            ...
            new_rles.append(rle_encode(tight))
    ...
    def with_offset(
        ...
        for mask_idx in range(num_masks):
            ...
            if ix1 > ix2 or iy1 > iy2:
                ...
                out_rles.append(rle_encode(np.zeros((1, 1), dtype=bool)))
            ...
            out_rles.append(rle_encode(clipped))
